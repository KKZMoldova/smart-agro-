...
app.get('/api/work-types',  async (req, res) => {
  try { const r = await db.query('SELECT * FROM work_types ORDER BY name ASC'); res.json(r.rows); }
...
app.get('/api/analyses/:id/pdfs', async (req, res) => {
  try {
    const r = await db.query(`SELECT id,filename,mimetype,size,uploaded_at FROM analysis_files WHERE analysis_id=$1 ORDER BY uploaded_at DESC`, [req.params.id]);
...
app.post('/api/analyses/:id/pdfs', express.raw({ type: 'application/pdf', limit: '5mb' }), async (req, res) => {
  try {
    ...
    const data = req.body;
    if (!data || !data.length) return res.status(400).json({ error: 'PDF не получен' });
    const r = await db.query(
      `INSERT INTO analysis_files (analysis_id,filename,mimetype,size,data) VALUES ($1,$2,$3,$4,$5) RETURNING id,filename,size,uploaded_at`,
...
app.get('/api/analyses/:id/pdfs/:fid/download', async (req, res) => {
  try {
    const r = await db.query(`SELECT filename,mimetype,data FROM analysis_files WHERE id=$1 AND analysis_id=$2`, [req.params.fid, req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'Не найден' });
    const f = r.rows[0];
    res.set({ 'Content-Type': f.mimetype, 'Content-Disposition': `attachment; filename="${encodeURIComponent(f.filename)}"` });
    res.send(f.data);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/analyses/:id/pdfs/:fid/base64', async (req, res) => {
  try {
    const r = await db.query(`SELECT filename,mimetype,data FROM analysis_files WHERE id=$1 AND analysis_id=$2`, [req.params.fid, req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'Не найден' });
    res.json({ ok: true, base64: Buffer.from(r.rows[0].data).toString('base64'), filename: r.rows[0].filename });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.delete('/api/analyses/:id/pdfs/:fid', async (req, res) => {
  try {
    await db.query(`DELETE FROM analysis_files WHERE id=$1 AND analysis_id=$2`, [req.params.fid, req.params.id]);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/import/orchard', async (req, res) => {
  try {
    const d = req.body;
    await db.query(`INSERT INTO settings (key,value) VALUES ('orchard_full_state',$1) ON CONFLICT (key) DO UPDATE SET value=$1, updated_at=NOW()`, [JSON.stringify(d)]);
    if (d.treatments?.length) {
      for (const t of d.treatments) {
        await db.query(
          `INSERT INTO treatments (id,date,parcel_name,products,method,volume,max_whi,whi_date,note) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO UPDATE SET date=$2,parcel_name=$3,products=$4,method=$5,volume=$6,max_whi=$7,whi_date=$8,note=$9`,
          [String(t.id), t.date, t.cellTarget||'all', JSON.stringify(t.products||[{name:t.product,type:t.type,dose:t.dose}]), t.method||'foliar', t.water||400, t.duration||14, t.endDate||null, t.note||'']
        );
      }
    }
...
app.post('/api/fields/polygon', async (req, res) => {
  try {
    ...
    await db.query(`INSERT INTO settings (key,value) VALUES ($1,$2) ON CONFLICT (key) DO UPDATE SET value=$2, updated_at=NOW()`, [`field_polygon_${field_id}`, JSON.stringify(polygon)]);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/fields/polygons', async (req, res) => {
  try {
    ...
    const polygons = {};
    r.rows.forEach(row => { polygons[row.key.replace('field_polygon_', '')] = typeof row.value === 'string' ? JSON.parse(row.value) : row.value; });
...
app.post('/api/state/vegetable', async (req, res) => {
  try {
    const { fields, equipment, parcels, treatments, catalog, warehouse, settings, sowingRecords, varieties } = req.body;
    const r = await db.query(`SELECT value FROM settings WHERE key='vegetable_full_state'`);
    ...
    await db.query(`INSERT INTO settings (key,value) VALUES ('vegetable_full_state',$1) ON CONFLICT (key) DO UPDATE SET value=$1, updated_at=NOW()`, [JSON.stringify(state)]);
    res.json({ ok: true });
...
cron.schedule('0 7 * * *', async () => {
  try {
    ...
    const state = typeof stateRes.rows[0].value === 'string' ? JSON.parse(stateRes.rows[0].value) : stateRes.rows[0].value;
    ...
    const tmax = parseFloat(latest.tmax)||20, tmin = parseFloat(latest.tmin)||10;
    const hum = parseFloat(latest.humidity)||50, lw = parseFloat(latest.leaf_wet)||0;
    const date = latest.date?.toString().slice(0,10);
    const smithActive = weather.length>=2 && weather.slice(0,2).every(w=>parseFloat(w.tmax)>=10&&parseFloat(w.humidity)>=80);
    const parcels = state.parcels||[];
    let alerts = [];
    if (smithActive) alerts.push('🟤 <b>SMITH PERIOD АКТИВЕН</b> — обработка томата и картофеля!');
    const tavg = (tmax+tmin)/2;
    if (tavg>=5&&tavg<=20&&lw>=4) { const pp=parcels.filter(p=>p.cropId==='pea').map(p=>p.name).join(', '); if(pp) alerts.push(`💜 <b>Пероноспороз гороха</b>\nУчастки: ${pp}\nОбработка: Ридомил Голд`); }
    if (hum>75&&lw>=3&&tavg>=12) { const pp=parcels.filter(p=>p.cropId==='pea').map(p=>p.name).join(', '); if(pp) alerts.push(`🟤 <b>Аскохитоз гороха</b>\nУчастки: ${pp}\nОбработка: Хорус`); }
    if (tmin<=2) alerts.push(`❄️ <b>ЗАМОРОЗОК</b> — T°мин ${tmin}°C`);
    if (tmax>=26) { const pp=parcels.filter(p=>p.cropId==='pea').map(p=>p.name).join(', '); if(pp) alerts.push(`🌡️ <b>Тепловой стресс гороха</b> — T°макс ${tmax}°C\n${pp}`); }
    const msg = alerts.length===0
      ? `✅ <b>Smart Agro · ${date}</b>\n\nДоброе утро! Алертов нет.\nT°мин ${tmin}°C / T°макс ${tmax}°C · RH ${hum}%`
      : `🌿 <b>Smart Agro · ${date}</b>\n\nT°мин ${tmin}° / T°макс ${tmax}° · RH ${hum}%\n\n⚠️ АЛЕРТЫ:\n\n${alerts.join('\n\n')}`;
...
setTimeout(() => {
  if (!process.env.FIELDCLIMATE_PUBLIC_KEY_ORCHARD && !process.env.FIELDCLIMATE_PUBLIC_KEY_VEG) { console.log('[CRON] FieldClimate keys not set'); return; }
