...
function buildMessage(task) {
  ...
  const chems = task.chemicals ? (typeof task.chemicals === 'string' ? JSON.parse(task.chemicals) : task.chemicals) : [];
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await db.query(
      ...
      [
        id,
        work_type_id||null, work_type_name||null, description||null,
        parcel_id||null, parcel_name||null,
        toJsonStr(parcels_json),
        total_ha||null,
        toJsonStr(chemicals),
...
router.patch('/:id/status', async (req, res) => {
  try {
    const {
      status, problem_note, closed_by_name,
      equipment_id, equipment_name, attachment,
      mechanic_id, mechanic_name, speed_kmh, note
    } = req.body;
    ...
    if (equipment_id)   { params.push(equipment_id);   q += `, equipment_id=$${params.length}`;
                          params.push(equipment_name||null); q += `, equipment_name=$${params.length}`; }
    ...
    if (mechanic_id)    { params.push(mechanic_id);    q += `, mechanic_id=$${params.length}`;
                          params.push(mechanic_name||null); q += `, mechanic_name=$${params.length}`; }
