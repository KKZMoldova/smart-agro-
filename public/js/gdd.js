...
const TECHMAP = {
  ...
  // Источники: Guy Sela "Fertilization and Irrigation" 2023 (общий вынос NPK, принципы фертигации);
  // Rutgers NJAES 2025/2026 Commercial Tree Fruit Production Guide, гл.10 Apples (фенология,
  // фунгицидные/инсектицидные программы по фазам, DD-модели плодожорки, некорневые Ca/B/Zn/Mn);
  // WSU Tree Fruit Extension, B. Sallato — "Fall Nutrient Sprays in Tree Fruit"
  // (осенние некорневые N/B/Zn, санитарный листопад мочевиной против парши).
  // ICL Growing Solutions — Apple Crop Guide (icl-growingsolutions.com/en-ca/agriculture/crops/apple/):
  // независимая сверка динамики K/N/Ca; принцип "K/N ratio важнее абсолютного N перед сбором"
  // (2 мес. до сбора) и связь избытка N + низкого K с горькой ямчатостью — совпадает с Rutgers/WSU.
  // ⚠️ ICL-таблица выноса NPK по факту сбора (≈224 кг N/га на ~45–49 т/га ≈ 4.5–5.0 кг N/т) даёт
  // почти вдвое более высокий вынос N на тонну, чем Sela (170 кг N/га на 60 т/га ≈ 2.83 кг N/т);
  // источник ICL — маркетинговая страница поставщика удобрений, внутренне противоречива (годовая
  // таблица выноса даёт ≈224 кг N/га, а помесячная таблица динамики на той же странице — ≈100 кг N/га),
  // поэтому её абсолютные нормы NPK сознательно НЕ приняты как основа, приоритет сохранён за Sela/Rutgers/WSU.
  // Открытый сад без плёночного укрытия — поле film описывает только погодные риски, не укрытие.
  crop_apple: [
    { phase:'Покой', bbch:'BBCH 00–01', period:'Ноябрь – Февраль', kc:0, film:'Открытый сад — укрытия нет', irrigation:'Не нужен', nutrition:'Нет',
      protection:['Искореняющая обработка Cu (медьсодержащий) по спящим почкам — снижает первичный запас парши и бактериальный ожог (Fire Blight) на побегах','Вырезка и сжигание веток с язвами бактериального ожога — САНИТАРИЯ ключевая для контроля болезни в новом сезоне (Rutgers 10.9)','Минеральное масло по спящим почкам — против яиц клещей и щитовки'],
      tasks:['Обрезка (декабрь–январь) — санитарная + формировочная, убрать всю мумифицированную падалицу','Анализ почвы — план внесения извести/удобрений на сезон','Ревизия шпалеры, опор, ловушек','Настроить метеостанцию и феромонные ловушки плодожорки к марту (биофикс для DD-модели)'],
      alert:'🔥 Не оставлять срезанные ветки с бактериальным ожогом в саду — источник заражения весной' },
    { phase:'Набухание почек', bbch:'BBCH 01–03 (Delayed Dormant – ½" Green)', period:'Начало марта', kc:0.15, film:'Открытый сад', irrigation:'Минимальный — 5–8 мм/неделю', nutrition:'Некорневая мочевина 2–3% при слабом N по листовому анализу прошлого года',
    ...
    { phase:'Мышиное ухо', bbch:'BBCH 10 (Tight Cluster, Pre-Pink)', period:'Конец марта', kc:0.35, film:'Открытый сад', irrigation:'10–12 мм/неделю', nutrition:'Ca хелат некорневая — старт программы против горькой ямчатости (bitter pit), продолжать до конца лета (Rutgers 10.3)',
      protection:['Пик первичных инфекций парши при влажной погоде — системный фунгицид (д.в. дифеноконазол, ципродинил) по прогнозу осадков','Zn хелат (EDTA) 1 л/100 л воды — при низком Zn по листовому анализу (Rutgers: Tight Cluster to Pink)','Начало учёта яиц/вылупления минирующей моли и клещей'],
      tasks:['Расставить ловушки на плодового пилильщика (European Apple Sawfly) — критично к цветению','Проверить смыв осадками — если >15–20мм, повторить обработку от парши'],
      alert:'☔ Затяжная влажная погода на этой фазе = максимальный риск сильной эпифитотии парши на сезон' },
    ...
    { phase:'Цветение', bbch:'BBCH 61–69 (Bloom)', period:'Апрель', kc:0.6, film:'Открытый сад', irrigation:'При заморозках — дождевание для защиты цветков, иначе не поливать', nutrition:'N мочевина 1% некорневая при слабом облиствении (не подменяет почвенное внесение)',
      protection:['🚫 ИНСЕКТИЦИДЫ ЗАПРЕЩЕНЫ — защита пчёл (Rutgers: "Do not apply insecticides during Bloom!")','Бактериальный ожог (Fire Blight) — КЛЮЧЕВАЯ угроза именно в цветение: стрептомициновый/биологический бактерицид по модели инфекции (T>15.5°C + влажность/дождь в открытых цветках)','ATS (тиосульфат аммония) 2% как прореживатель цветков и вспомогательное средство против ожога (Rutgers 10.5.1)'],
      tasks:['Ежедневно проверять модель риска бактериального ожога по погоде','Фиксировать даты начала/конца цветения по сортам — для калибровки GDD','Следить за лётом пчёл — не менее 5 пчёл/дерево при осмотре'],
      regulators:['Perlan/Promalin (6-BA + гиббереллины A4+A7) — улучшение формы плода, обработка при раскрытии королевских цветков; не применять при прогнозе дождя в ближайшие 6ч (BC Tree Fruit Guide)'],
      alert:'🔥 Дождь/град в открытые цветки при тепле >15.5°C = острый риск бактериального ожога, реагировать в течение 24ч' },
    { phase:'Завязь', bbch:'BBCH 71–75 (Petal Fall)', period:'Конец апреля – май', kc:0.75, film:'Открытый сад', irrigation:'15–20 мм/неделю', nutrition:'Ca(NO₃)₂ через капельницу или некорневая CaCl₂ 0.3% каждые 7–10 дней — старт основной программы против bitter pit (≈4.5–16 кг Ca/га за сезон, Rutgers 10.3)',
      protection:['Плодожорка яблонная — первая обработка через 2–3 недели после лепестков по DD-модели (250 DD база 10°C от биофикса)','Долгоносик-казарка (Plum Curculio) — критическое окно с лепестков до 2-го опрыскивания','Фунгицид против парши продолжается при влажной погоде, добавить защиту от мучнистой росы'],
      tasks:['Начать нормировку урожая (химическое или ручное прореживание) как можно раньше — влияет на размер плода','Продолжить мониторинг ловушек плодожорки и казарки','Оценить нагрузку урожаем по сортам'],
      regulators:['Химическое прореживание (7–25 дней после полного цветения): NAA (Fruitone L/N) при завязи 8–10мм и T≥10°C, лучше в пасмурную влажную погоду — ⚠️ ОСТОРОЖНО с Red Delicious (риск карликовых "pygmy" плодов при передозировке, BC Tree Fruit Guide)','Sevin XLR (карбарил) 10–25 дней после цветения при завязи 10–15мм и тёплой солнечной погоде (T≥20°C ближайшие 5 дней) — не более 1 обработки/сезон, вредит хищным клещам','MaxCel/Cilis Plus (6-BA) от опадения лепестков (6мм) до 10мм — усиливает эффект карбарила и размер плода; та же осторожность с Red Delicious при смеси с NAA','Accede (ACC) при 15–20мм — вспомогательный инструмент, если основное прореживание недостаточно, не замена основной программе','Apogee (пропогексадион-Ca) — регулятор вегетативного роста + подавление ВТОРИЧНОГО бактериального ожога (побеговая форма, не цветковая): первая обработка при 5см прироста побега, далее каждые 14–21 дней, максимум 4 обработки/сезон'],
      alert:'🐛 Долгоносик-казарка активен от лепестков до 1–2 покровных обработок — не пропустить окно' },
    { phase:'Рост плода', bbch:'BBCH 75–79 (First–Late Covers)', period:'Июнь – начало августа', kc:1.0, film:'Открытый сад', irrigation:'20–30 мм/неделю в жару, пульсирующий полив 2–3 цикла/день на лёгких почвах', nutrition:'K (сульфат калия) 8–12 кг/га на налив + продолжение Ca-программы (хелат каждые 7–10 дней). N по остаточной потребности, не форсировать. Mg — только при подтверждённом дефиците: некорневая в июне, при необходимости повторить в июле; ⛔ НЕ применять после 1 августа (WSU Crop Protection Guide)',
      ...
      tasks:['Ротация препаратов по FRAC/IRAC группам — не более 2 обработок подряд одним МОА (риск резистентности парши и плодожорки)','Контроль летней обрезки/пасынкования для проветривания кроны (снижает сажистый налёт)','Подготовить сети/отпугиватели от птиц к созреванию ранних сортов'],
    ...
    { phase:'Созревание', bbch:'BBCH 81–89', period:'Август – сентябрь (по сорту)', kc:0.9, film:'Открытый сад', irrigation:'15–20 мм/неделю, избыток воды снижает Brix и лёжкость', nutrition:'⛔ Стоп N за 4–6 недель до сбора. Соотношение K/N важнее абсолютного количества N и должно быть максимально высоким в последние 1.5–2 месяца перед сбором (ICL Growing Solutions) — избыток N при низком K повышает риск горькой ямчатости так же, как и дефицит Ca. Ca-обработки продолжать до последней недели — главная защита от горькой ямчатости в хранении',
      protection:['Только препараты с коротким сроком ожидания (PHI ≤ 7–14 дней по этикетке) при необходимости','Контроль сажистого налёта/мухоседа перед сбором — влияет на товарный вид'],
      tasks:['Тест на крахмал/иодную пробу и Brix по срокам сорта для определения оптимальной даты сбора','Проверка холодильных камер и тары','Планирование очерёдности сбора по срокам созревания сортов'],
      regulators:['ReTain (AVG) — стоп-дроп за 4 недели до ожидаемого сбора: задерживает предуборочное опадение на ~10 дней, замедляет размягчение и распад крахмала, снижает загар при хранении в РА; ⚠️ НЕ смешивать с NAA/этефоном (противоположный эффект на этилен)','BLUSH (прогидроджазмон) — усиление окраски, 100–200ppm, 1–2 обработки с интервалом 7–14 дней за 7–28 дней до сбора, лучше утром/вечером в сухую погоду с длительным просыханием'],
      alert:'🍎 Ранний или поздний сбор относительно оптимума = главный фактор лёжкости и риска побурения мякоти' },
    { phase:'После сбора', bbch:'BBCH 91–97', period:'Сентябрь – октябрь', kc:0.6, film:'Открытый сад', irrigation:'Снижать постепенно до конца октября, затем прекратить',
      nutrition:'Осенняя некорневая N: мочевина 1.0–1.2 кг д.в. N/100 л рабочего раствора, после сбора но ДО начала естественного листопада (пока листья ещё зелёные/желтеют), при 20–29°C, влажно, без сильного ветра — усвоение 30–80%. Вносить только если N по листу <1.7% или было тяжёлое плодоношение (при 1.7–2.5% — не нужно). Осенний B — второе из двух предпочтительных окон WSU (наравне с розовым бутоном): та же поддерживающая норма 0.56 кг B/га/год (до 1.12 кг/га при дефиците, максимум 48 г/100 л раствора), некорневая, НЕ фертигация (см. фаза "Розовый бутон"). Осенний Zn (яблоня, сульфат цинка): поддерживающая норма 2.2–4.5 кг Zn/га/год; при подтверждённом дефиците по листу (<15 ppm) — разовая коррекция 10.1 кг Zn/га сразу после сбора. Разовая осенняя обработка цинк в листьях в целом не поднимает, только в почках/цветках — нужны регулярные обработки в течение сезона, не только осенью. Смешивание микроэлементов с мочевиной улучшает усвоение (WSU Tree Fruit, Sallato).',
