...
let S = {
  ...
  varieties:[
    ...
    {id:'va1',name:'Granny Smith',  ripening:'Поздний', pollType:'cross',pollinators:'Golden Delicious, Red Delicious',color:'vc3',note:'GDD 1750–1950. Высокий риск жары, мучнистой росы, fire blight. Зелёный, поздний сбор. Подтверждено (UC Davis): TA на съём ≤0.75%, очень восприимчив к горькой ямчатости и загару (scald) — Ca-дип 3–4% CaCl₂ хлопья (77%) или 2–3% обычный CaCl₂ перед хранением. ⚠️ Официальной модели РОСТА ПЛОДА у WSU/WTFRC для этого сорта нет (Hoogenboom et al., Acta Hort. 1160 — только Cripps Pink/Gala/Red Delicious) — только модель цветения (база 6.11°C, среднесуточная T° — метод совпадает с нашим движком, база выше), пороги стадий для Granny Smith не опубликованы; наш GDD 1750–1950 (база 4.5°C) — калибровка проекта, не WSU-источник. ⚠️ Regione Veneto: именно на Granny Smith в последние годы контроль серой яблонной тли (Dysaphis plantaginea, dc29) вызывал наибольшие сложности среди сортов — усиленный мониторинг от розового бутона. 📏 Целевой прирост однолетнего побега у зрелого дерева: 30–40 см (неспуровый тип, BC Tree Fruit Guide) — используется для контроля адекватности N: меньше нормы = вероятный дефицит N, больше = избыток N/риск слабой окраски и ямчатости.',cropId:'crop_apple'},
    {id:'va2',name:'Red Delicious', ripening:'Средний', pollType:'cross',pollinators:'Golden Delicious, Jonagold',     color:'vc1',note:'GDD 1650–1850. Критичен цвет (ночные T° <18°C) и Ca-программа против bitter pit. Подтверждено (Hoogenboom et al., Acta Hort. 1160/29+48, база на среднесуточной T° — тот же метод, что у нашего движка, но другая база): цветение — база 6.11°C, Полное цветение 234–333 / Опадение лепестков 291–361 GDD(°F, /1.8 для °C) от зелёного конуса; рост плода после полного цветения — своя база 6.11°C, модель Von Bertalanffy предсказывает диаметр плода с ошибкой <3%.',cropId:'crop_apple'},
    {id:'va3',name:'Jonagold',      ripening:'Средний', pollType:'cross',pollinators:'Golden Delicious, Granny Smith', color:'vc4',note:'GDD 1700–1900. Сильнорослый. Главные риски: fire blight (5) и парша (4). ATS-прореживание: чувствителен к дозам выше 1.2 л/100л (BC Tree Fruit Guide). ℹ️ По остальной проработанной литературе (ICL/WSU/BC/ERSA FVG/Regione Veneto) сорт-специфичных данных для Jonagold не найдено — эти источники фокусируются на Gala/Granny Smith/Red Delicious/Golden Delicious/Fuji/Pink Lady; технокарта опирается на общие для яблони нормы.',cropId:'crop_apple'},
    {id:'va4',name:'Braeburn',      ripening:'Поздний', pollType:'cross',pollinators:'Granny Smith, Jonagold',         color:'vc5',note:'GDD 1800–2000. Поздний. Парша (5), мучнистая роса (4), стресс жары (4). ATS НЕ применять (низкая завязываемость, BC Tree Fruit Guide). ⚠️ NAD/NAA фитотоксичен на этом сорте (ERSA FVG) — для прореживания использовать 6-BA/Metamitron вместо ауксинов.',cropId:'crop_apple'},
    {id:'va5',name:'Gala',          ripening:'Ранний',  pollType:'cross',pollinators:'Golden Delicious, Granny Smith', color:'vc2',note:'GDD 1550–1750 (калибровка проекта по аналогии с другими сортами — уточнить по факту сезона). Подтверждено (Hoogenboom et al., Acta Hort. 1160/29+48, среднесуточная T° — тот же метод, что у нашего движка): цветение — база 6.11°C (Полное цветение 243–329 / Опадение лепестков 298–365 GDD(°F, /1.8 для °C) от зелёного конуса); рост плода после полного цветения — СВОЯ база 5.55°C (ниже, чем у Ред Делишес/Криппс Пинк — из-за более раннего сбора), модель Von Bertalanffy предсказывает диаметр плода с ошибкой <3%. ⚠️ Мучнистая роса повышенного риска (ERSA FVG группирует с Granny Smith). Альтернариоз — риск уже с завязи, симптомы на плодах с середины мая (dc27). NAD не применять на молодых деревьях (ERSA FVG). Metamitron (Brevis) — предпочтительный прореживатель для Gala. ReTain снижает растрескивание плодов, НО может задержать развитие красной окраски в отдельные годы (BC Tree Fruit Guide) — учитывать при позднем сборе для рынка, где важен цвет. 📏 Целевой прирост однолетнего побега: 25–30 см (спуровый тип, BC Tree Fruit Guide).',cropId:'crop_apple'},
  ],
  catalog:[
  ...
  crops: [
    ...
    },
    {
      ...
      diseases:['dc8','dc9','dc10','dc20','dc21','dc22','dc23','dc24','dc25','dc26','dc27','dc28','dc29'], // refs to diseaseCatalog — парша, мучнистая роса, плодожорка, бактериальный ожог, плодовая муха, казарка, пилильщик, минирующая моль, сажистый налёт/мухосед, шерстистая тля, альтернариоз, калифорнийская щитовка, серая тля (полная модель риска)
      pests:[
  ...
  gddDb: {
    ...
    varietySensitivity: {
      ...
      'va5': {'dc3':3,'dc4':5,'dc5':3}, // Gala — мучнистая роса повышена (ERSA FVG группирует с Granny Smith: "Inored, Granny smith, Gala")
  ...
  diseaseCatalog:[
    ...
    {id:'dc10',name:'Плодожорка яблонная',latin:'Cydia pomonella',type:'pest',danger:4,cropIds:['crop_apple'],
     ...
     products:'Калипсо, Декис, Димилин, Матч; ротация по FRAC/IRAC — хлорантранилипрол (макс. 2 обработки, интервал 12-14 дней, предпочтительно 1-е поколение), эмамектин (до 3 обработок, 2-3-е поколение), спинеторам (1 обработка/сезон)',
     prevention:'Феромонные ловушки с апреля, вешать высоко в кроне (на шесте/блоке). Обработка при 250 GDD (первый лёт). ⚠️ Лёт активен только когда сумеречная T°>15-16°C — даже при достигнутом GDD-пороге без тёплых сумерек лёта не будет (Regione Veneto). Хозяйства с ущербом >2% урожая в прошлом году — зона повышенного риска, усилить программу (Regione Veneto). Биопрепараты: вирус гранулёза наиболее эффективен именно на 1-м поколении (начало мая — середина июня) — при более низких T° личинка дольше внедряется в плод и успевает проглотить больше вируса; энтомопатогенные нематоды осенью против зимующих личинок — нужны T>10-12°C, длительное увлажнение, слабая освещённость (Regione Veneto).'},
    {id:'dc20',name:'Бактериальный ожог (Fire Blight)',latin:'Erwinia amylovora',type:'bacterial',danger:5,cropIds:['crop_apple'],
     tmin:15,tmax:32,topt:24,hmin:60,rainMin:1,leafWetHours:2,rainDays:1,
     criticalPhases:['Розовый бутон','Цветение'],phase:'flowering',
     symptoms:'Внезапное почернение и увядание цветков и молодых побегов ("обожжённый" вид), крючкообразно загнутые верхушки (shepherd\'s crook). Камедетечение и язвы на коре.',
     products:'Стрептомицин (где разрешён), Косайд (гидроксид меди), биопрепараты (Blossom Protect, Серенейд)',
     prevention:'Вырезка и сжигание язв зимой (санитария — ключевой контроль). Медьсодержащий по спящим почкам. Риск максимален при T>15.5°C + дождь/влага в открытые цветки — реагировать в течение 24ч. ⚠️ Избыток N усиливает восприимчивость дерева к ожогу (чрезмерный вегетативный рост, BC Tree Fruit Guide) — не форсировать N весной.'},
    {id:'dc21',name:'Яблонная плодовая муха (Apple Maggot)',latin:'Rhagoletis pomonella',type:'insect',danger:4,cropIds:['crop_apple'],
     tmin:18,tmax:32,topt:25,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Рост плода','Созревание'],phase:'all',
     symptoms:'Извилистые ходы личинок под кожицей и в мякоти плода, ямчатая деформация мякоти, преждевременное опадение и загнивание плода изнутри.',
     products:'Калипсо, Аваунт, Ассейл',
     prevention:'Жёлтые/красные сферические липкие ловушки с конца июня–июля. Порог 2–5 мух/ловушку/неделю.'},
    {id:'dc22',name:'Долгоносик-казарка (Plum Curculio)',latin:'Conotrachelus nenuphar',type:'insect',danger:4,cropIds:['crop_apple'],
     tmin:10,tmax:27,topt:18,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Завязь'],phase:'all',
     symptoms:'Серповидный шрам-надрез на кожице молодого плода (яйцекладка). Деформация и преждевременное опадение поражённых завязей.',
     products:'Ассейл, Имидан, Аваунт',
     prevention:'Обработка сразу после опадения лепестков — критическое окно до 1–2-й покровной обработки.'},
    {id:'dc23',name:'Плодовый пилильщик яблонный (European Apple Sawfly)',latin:'Hoplocampa testudinea',type:'insect',danger:3,cropIds:['crop_apple'],
     tmin:12,tmax:22,topt:17,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Цветение','Завязь'],phase:'all',
     symptoms:'Извилистый поверхностный ход на кожице молодого плода с ржавыми экскрементами, позже — глубокий ход к сердцевине, поражённый плод опадает.',
     products:'Калипсо, Актара — сразу после опадения лепестков',
     prevention:'Белые клеевые ловушки в цветение для мониторинга лёта. Обработка до откладки яиц вглубь плода.'},
    {id:'dc24',name:'Минирующая моль (Spotted Tentiform Leafminer)',latin:'Phyllonorycter blancardella',type:'insect',danger:2,cropIds:['crop_apple'],
     tmin:12,tmax:30,topt:22,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Мышиное ухо','Розовый бутон'],phase:'all',
     symptoms:'Пятна-мины на листьях от питания личинок между слоями эпидермиса. При массовом поражении — снижение фотосинтеза и качества плода.',
     products:'Агри-мек, Проклэйм, минеральное масло (по яйцам)',
     prevention:'Учёт яиц/вылупления с фазы "Мышиное ухо". Часто природные энтомофаги сдерживают без обработок — обрабатывать только при превышении порога.'},
    {id:'dc25',name:'Сажистый налёт и мухосед (Sooty Blotch & Flyspeck)',latin:'Комплекс грибов (Peltaster, Zygophiala и др.)',type:'fungal',danger:2,cropIds:['crop_apple'],
     tmin:18,tmax:30,topt:24,hmin:80,rainMin:2,leafWetHours:10,rainDays:3,
     criticalPhases:['Рост плода','Созревание'],phase:'all',
     symptoms:'Тёмные "сажистые" пятна грибницы и мелкие чёрные точки (мухосед) на кожице плода. Мякоть не повреждается, но резко страдает товарный вид.',
     products:'Каптан, Топсин, стробилурины (профилактически в покровные обработки)',
     prevention:'Проветривание кроны (летняя обрезка/пасынкование) снижает влажность в кроне. Покровные фунгициды перед сбором в затяжную влажную погоду.'},
    {id:'dc26',name:'Шерстистая тля (Woolly Apple Aphid)',latin:'Eriosoma lanigerum',type:'insect',danger:3,cropIds:['crop_apple'],
     tmin:15,tmax:30,topt:23,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Завязь','Рост плода'],phase:'all',
     symptoms:'Белые ватообразные колонии на штамбе, корневой шейке и в кроне (у ран/срезов). Вздутия и наросты (галлы) на поражённых побегах и корнях, ослабление дерева.',
     products:'Спиротетрамат (1 обработка/год — предпочтительно на миграцию), ацетамиприд, флупирадифуран',
     prevention:'Порог обработки: 10 колоний на 100 осмотренных органов (ERSA FVG). Мониторинг штамба и корневой шейки, не только кроны. Перед обработкой оценить степень естественного паразитирования Aphelinus mali (наездник) — вмешательство часто запаздывает и становится заметным только к июлю, когда ущерб уже нанесён (Regione Veneto). Миграция на молодой прирост — первая декада мая, лучшее окно для химобработки. Снизить N, полив и загущенность кроны — избыточная вегетативная сила усиливает заселение (Regione Veneto).'},
    {id:'dc27',name:'Альтернариоз (Alternaria leaf blotch/fruit rot)',latin:'Alternaria alternata',type:'fungal',danger:2,cropIds:['crop_apple'],
     tmin:20,tmax:32,topt:26,hmin:80,rainMin:2,leafWetHours:8,rainDays:2,
     criticalPhases:['Завязь','Рост плода'],phase:'all',
     symptoms:'Некротические пятна на листьях (5-10мм, бурые→серебристые — легко спутать с дефицитом Mg!) и на плодах (0.5-1мм, бурые/чёрные с красноватым ореолом); на восприимчивых сортах (Gala, Golden Delicious, Pink Lady) может поражать и плоды в хранении.',
     products:'Флуазинам, SDHI (флуксапироксад/пентиопирад — попутный эффект при обработке от парши), дитианон',
     prevention:'Риск существует уже с завязи на восприимчивых сортах: на Gala симптомы на плодах уже с середины мая (Regione Veneto — самый ранний срок среди сортов). Отдельная обработка обычно не нужна — препараты от парши (флуазинам/SDHI/дитианон) закрывают и эту угрозу. Избегать дождевания по кроне, поливать рано утром — избыточная влажность кроны усиливает риск.'},
    {id:'dc28',name:'Калифорнийская щитовка (San Jose Scale)',latin:'Quadraspidiotus perniciosus',type:'insect',danger:3,cropIds:['crop_apple'],
     tmin:8,tmax:25,topt:18,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Набухание почек','Зелёный конус'],phase:'all',
     symptoms:'Мелкие круглые щитки на коре, ветвях и плодах; вокруг места питания — красноватый ореол на плодах и коре. Ослабление и усыхание ветвей при сильном заражении.',
     products:'Минеральное масло + сера (до 3 л масла + 1 кг серы/100л) по спящим/набухающим почкам',
     prevention:'Окно обработки — от набухания почек до разрозненных бутонов (mazzetti divaricati), по зимующим неонидам. Критично равномерное смачивание кроны — работать в безветренную, сухую, тёплую погоду (Regione Veneto). Позже в сезон эффективных вариантов защиты намного меньше — не пропустить это окно.'},
    {id:'dc29',name:'Серая яблонная тля (Rosy Apple Aphid)',latin:'Dysaphis plantaginea',type:'insect',danger:4,cropIds:['crop_apple'],
     tmin:10,tmax:25,topt:18,hmin:0,rainMin:0,leafWetHours:0,rainDays:0,
     criticalPhases:['Розовый бутон','Цветение','Завязь'],phase:'all',
     symptoms:'Скручивание и деформация листьев на концах побегов с колониями тли внутри. Поражённые плодики остаются мелкими и уродливыми — самый вредоносный вид тли на яблоне.',
     products:'Флоникамид или тау-флувалинат (розовый бутон — ключевая обработка); после цветения при новых колониях — ацетамиприд, сульфоксафлор, флупирадифуран, спиротетрамат или флоникамид',
     prevention:'⚠️ Granny Smith — сорт с наибольшими сложностями контроля этого вредителя в наблюдениях последних лет (Regione Veneto) — усиленный мониторинг и не затягивать с обработкой именно на этом сорте. Ключевое окно — розовый бутон (до раскручивания листьев с колониями внутри) и первые 3-4 недели после цветения.'},
  ],
