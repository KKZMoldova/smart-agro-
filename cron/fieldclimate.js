...
function parseResponse(json, label) {
  ...
  keys.forEach(function(k) {
    const s = data[k];
    const vkeys = s && s.values ? Object.keys(s.values).join(',') : 'no values';
    console.log('[FC] sensor ' + k + ': ' + (s && s.name || '?') + ' | values keys: ' + vkeys);
  });

  function findSensor(nameFragments, valueKey) {
    for (var fi = 0; fi < nameFragments.length; fi++) {
      var frag = nameFragments[fi];
      for (var ki = 0; ki < keys.length; ki++) {
        var s = data[keys[ki]];
        var name = (s && s.name || '').toLowerCase();
        if (name.indexOf(frag.toLowerCase()) >= 0) {
  ...
  var vTmax  = findSensor(['HC Air temperature', 'air temp', 'temperatura'], 'max');
  var vTmin  = findSensor(['HC Air temperature', 'air temp', 'temperatura'], 'min');
  var vTavg  = findSensor(['HC Air temperature', 'air temp', 'temperatura'], 'avg');
  var vHum   = findSensor(['HC Relative humidity', 'relative hum', 'humidity'], 'avg');
  var vRain  = findSensor(['Precipitatii', 'Precipitation', 'rain', 'осадки'], 'sum');
  var vSolar = findSensor(['Solar radiation', 'solar rad', 'global rad'], 'avg');
  var vLeaf  = findSensor(['Leaf Wetness', 'leaf wet', 'Влажность листьев'], 'time');
  var vEt0   = findSensor(['ET0', 'et0', 'ETO'], 'result');
